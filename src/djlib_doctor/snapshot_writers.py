...
def write_missing_files(
    path: Path, library: RekordboxLibrary, tracks: tuple[Track, ...], redact_paths: bool = False
) -> None:
    ...
    rows = [
        {
            ...
            "path": display_path(track.path, redact_paths),
        }
        for track in tracks
    ]
...
def write_streaming_placeholders(path: Path, library: RekordboxLibrary, redact_paths: bool = False) -> None:
    rows = [
        ...
        if track.location_kind is LocationKind.STREAMING_PLACEHOLDER
    ]
        ...
        rows.append(
            {
                ...
                "path": display_path(track.path, redact_paths),
                "local_exists": "yes" if _local_exists(track) else "no",
                "cue_count": len(track.cues),
    ...
    rows = [
        {
            ...
            "missing_collection_refs": len([track_id for track_id in playlist.entries if track_id not in track_ids]),
            "missing_track_ids": " | ".join(track_id for track_id in playlist.entries if track_id not in track_ids),
        }
...
def write_filesystem_inventory(path: Optional[Path], music_root: Path, redact_paths: bool = False) -> dict[str, Any]:
    ...
    for audio_file in iter_audio_files(music_root):
        ...
        total_bytes += size
        rows.append(
            {"path": display_path(audio_file, redact_paths), "extension": audio_file.suffix.lower(), "size_bytes": size}
        )
...
def redact_report(report: VerificationReport) -> VerificationReport:
    ...
    findings = [
        replace(
            finding,
            path=redact_path(finding.path) if finding.path else "",
            message=redact_text_path(finding.message, finding.path) if finding.path else finding.message,
        )
        for finding in report.findings
    ]
    return replace(
        report, source_path=redact_path(report.source_path), missing_local_files=missing, findings=tuple(findings)
    )
