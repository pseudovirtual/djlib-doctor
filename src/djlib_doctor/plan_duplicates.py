...
def build_duplicates_plan(
    snapshot_path: Path, collision_policy: DuplicateCollisionPolicy = DuplicateCollisionPolicy.CUE_SAFE
) -> PlanReport:
    artifacts = read_json(snapshot_path).get("artifacts", {})
    rows = [
        row
        for row in read_csv(artifact_path(snapshot_path, artifacts["track_summary_csv"]))
        if row.get("location_kind") != "streaming_placeholder"
    ]
...
def _duplicate_actions(
    group_id: str, group: list[dict[str, str]], policy: DuplicateCollisionPolicy
) -> list[PlanAction]:
...
def _duplicate_decision(
    row: dict[str, str], survivor: dict[str, str], multiple_cued: bool, policy: DuplicateCollisionPolicy
):
    if policy is DuplicateCollisionPolicy.KEEP_BOTH:
        return (
            "keep_duplicate_record",
            MatchConfidence.CANDIDATE,
            "Collision policy says to keep both duplicate records until the DJ makes a manual decision.",
        )
    if row.get("track_id") == survivor.get("track_id"):
        return (
            "keep_duplicate_survivor",
            MatchConfidence.CANDIDATE,
            f"Recommended survivor: {describe_duplicate_collision_policy(policy)}",
        )
    if multiple_cued:
        return (
            "review_multiple_cued_duplicate",
            MatchConfidence.UNSAFE,
            "Multiple duplicate records have cues; cue union or manual review is required before removing any record.",
        )
    if policy is DuplicateCollisionPolicy.QUALITY and int_value(row.get("cue_count")) > 0:
        return (
            "review_cue_migration_before_removing_duplicate",
            MatchConfidence.UNSAFE,
            "Quality-first policy selected a different survivor; cue migration must be reviewed before removing this record.",
        )
    return (
        "review_remove_duplicate_later",
        MatchConfidence.CANDIDATE,
        "Duplicate non-survivor can be reviewed after survivor and playlist/cue preservation are verified.",
    )
