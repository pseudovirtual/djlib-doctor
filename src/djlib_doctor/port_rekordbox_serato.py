...
def write_rekordbox_to_serato_plan(plan: RekordboxToSeratoPlan | RekordboxToSeratoBatchPlan, out_dir: Path) -> dict[str, str | list[str]]:
    ...
    if isinstance(plan, RekordboxToSeratoBatchPlan):
        ...
        return {"manifest": str(manifest_path), "crate_previews": [str(path) for path in crate_paths], "unsupported_csv": str(unsupported_path)}
...
def read_playlist_names(path: Path) -> tuple[str, ...]:
    return tuple(line.strip() for line in path.read_text(encoding="utf-8").splitlines() if line.strip() and not line.strip().startswith("#"))


def render_rekordbox_to_serato_summary(plan: RekordboxToSeratoPlan | RekordboxToSeratoBatchPlan) -> str:
    summary = plan.summary
    lines = ["Dry-run Serato summary", f"Crates: {summary.get('crates', 1)}", f"Tracks: {summary['tracks']}", f"Skipped: {summary['skipped']}", f"Cue intents: {summary['cue_intents']}", f"Warnings: {summary['warnings']}"]
...
def _write_unsupported_csv(path: Path, plan: RekordboxToSeratoPlan | RekordboxToSeratoBatchPlan) -> None:
    ...
    for crate_plan in plans:
        rows.extend({"track_id": track.source_id, "artist": track.artist, "title": track.title, "issue": issue} for track in crate_plan.tracks for issue in track.unsupported)
        rows.extend({"track_id": row.get("track_id", ""), "artist": row.get("artist", ""), "title": row.get("title", ""), "issue": row.get("reason", "")} for row in crate_plan.skipped)
