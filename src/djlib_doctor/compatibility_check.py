...
def check_audio_compatibility(
    probe: AudioProbe, profile: AudioCompatibilityProfile = REKORDBOX_CONSERVATIVE_PROFILE
) -> CompatibilityResult:
    ...
    if not probe.probe_ok:
        issues.append(
            _issue(
                "probe_failed",
                CompatibilitySeverity.FAILURE,
                "Audio probe did not complete; compatibility cannot be trusted.",
            )
        )
    if probe.extension.lower() and probe.extension.lower() not in profile.allowed_extensions:
        issues.append(
            _issue(
                ...
                CompatibilitySeverity.FAILURE,
                f"Extension {probe.extension.lower()} is outside the {profile.name} profile.",
            )
        )
    if not probe.codec:
        ...
        issues.append(
            _issue(
                "unsupported_codec",
                CompatibilitySeverity.FAILURE,
                f"Codec {probe.codec.lower()} is outside the {profile.name} profile.",
            )
        )
    if probe.sample_rate_hz is not None and probe.sample_rate_hz > profile.max_sample_rate_hz:
        issues.append(
            _issue(
                ...
                CompatibilitySeverity.FAILURE,
                f"Sample rate {probe.sample_rate_hz} Hz exceeds {profile.max_sample_rate_hz} Hz.",
            )
        )
    if probe.bit_depth is not None and probe.bit_depth > profile.max_bit_depth:
        issues.append(
            _issue(
                "bit_depth_too_high",
                CompatibilitySeverity.FAILURE,
                f"Bit depth {probe.bit_depth} exceeds {profile.max_bit_depth}.",
            )
        )
    ...
    if probe.bit_rate_kbps is not None and 0 < probe.bit_rate_kbps < profile.warn_below_bit_rate_kbps:
        issues.append(
            _issue(
                ...
                f"Bit rate {probe.bit_rate_kbps} kbps is below {profile.warn_below_bit_rate_kbps} kbps.",
            )
        )
