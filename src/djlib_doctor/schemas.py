...
SCHEMAS: dict[str, dict[str, Any]] = {
    ...
    "detection": _json_schema(
        DETECT_SCHEMA_VERSION,
        ("schema_version", "home", "volumes", "summary", "findings"),
        finding_fields=["platform", "kind", "path"],
    ),
    "snapshot": _json_schema(
    ...
    "compare": _json_schema(
        COMPARE_SCHEMA_VERSION,
        ("schema_version", "status", "summary", "issues"),
        issue_codes=[
            "missing_material",
            "cue_not_covered",
            "hotcue_regression",
    ...
    "review-log": _json_schema(
        REVIEW_SCHEMA_VERSION,
        ("schema_version", "source_plan_type", "generated_at", "summary", "decisions"),
        decision_fields=["review_id", "decision", "notes", "action"],
    ),
    "apply-manifest": _json_schema(
        APPLY_MANIFEST_SCHEMA_VERSION,
        ("schema_version", "mode", "source_plan_type", "summary", "safety", "post_apply_verification", "operations"),
    ...
    "fingerprint-comparison": _json_schema(
        FINGERPRINT_SCHEMA_VERSION,
        (
            "schema_version",
            "comparison_basis",
            "claims_audio_identity",
            "classification",
            "byte_similarity",
            "left",
            "right",
        ),
        classification_values=["exact_duplicate", "byte_similar", "different"],
    ),
    "fingerprint-manifest": _json_schema(
        FINGERPRINT_SCHEMA_VERSION, ("schema_version", "root", "redacted_paths", "file_count", "files")
    ),
    "certification": _json_schema(
        CERTIFICATION_SCHEMA_VERSION,
        CERTIFICATION_FIELDS,
        severity_values=["info", "warning", "error"],
        summary_fields=["matched_tracks", "unmatched_tracks", "cues", "loops", "playlists", "unsupported_rows"],
    ),
    "serato-inspection": _json_schema(
        SERATO_INSPECTION_SCHEMA_VERSION,
        ("schema_version", "source", "summary", "schema_fingerprint", "tables", "asset_identity"),
    ),
    "port-manifest": _json_schema(
    ...
    "rekordbox-port-manifest": _json_schema(
    ...
    "serato-stage-manifest": _json_schema(
    ...
    "serato-install-report": _json_schema(
        SERATO_INSTALL_SCHEMA_VERSION, ("schema_version", "passed", "stage_manifest", "backup_dir", "installed_files")
    ),
    "serato-audio-tag-stage-manifest": _json_schema(
        SERATO_AUDIO_TAG_STAGE_SCHEMA_VERSION,
        (
            "schema_version",
            "mode",
            "safety",
    ...
    "serato-audio-tag-install-report": _json_schema(
        SERATO_AUDIO_TAG_INSTALL_SCHEMA_VERSION,
        ("schema_version", "passed", "stage_manifest", "backup_dir", "installed"),
    ),
    "file-operations-stage-manifest": _json_schema(
        FILE_OPS_STAGE_SCHEMA_VERSION,
        ("schema_version", "mode", "source_manifest", "operations", "install_token", "safety"),
        mode_values=["staged_file_operations"],
    ),
    "file-operations-install-report": _json_schema(
        FILE_OPS_INSTALL_SCHEMA_VERSION, ("schema_version", "passed", "stage_manifest", "backup_dir", "applied")
    ),
    "rekordbox-db-import-operations": _json_schema(
        IMPORT_SCHEMA_VERSION,
        ("schema_version", "mode", "source_port_manifest", "target_table", "summary", "operations"),
        mode_values=["rekordbox_db_import_operations"],
    ),
    "rekordbox-db-stage-manifest": _json_schema(
    ...
    "rekordbox-db-install-report": _json_schema(
        SQLITE_INSTALL_SCHEMA_VERSION, ("schema_version", "passed", "stage_manifest", "backup", "installed_db")
    ),
}
...
def render_schema(name: str | None = None, pretty: bool = False) -> str:
    data = (
        get_schema(name) if name else {"schemas": {schema_name: SCHEMAS[schema_name] for schema_name in schema_names()}}
    )
