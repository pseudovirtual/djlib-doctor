...
def write_crate_to_sqlite(conn: sqlite3.Connection, crate_name: str, tracks: tuple[dict[str, Any], ...], revision: int, now: int) -> tuple[int, int]:
...
def _upsert_container(conn: sqlite3.Connection, crate_name: str, revision: int, now: int) -> int:
    ...
    existing = conn.execute("SELECT id FROM container WHERE parent_id = ? AND name = ? COLLATE NOCASE AND type = 1", (SERATO_LIBRARY_ROOT_CONTAINER_ID, crate_name)).fetchone()
    if existing:
        container_id = int(existing[0])
        dynamic_update(conn, "container", {"revision": revision}, "id = ?", (container_id,))
        return container_id
    next_order = conn.execute("SELECT COALESCE(MAX(list_order), 0) + 1 FROM container WHERE parent_id = ?", (SERATO_LIBRARY_ROOT_CONTAINER_ID,)).fetchone()[0]
    return dynamic_insert(conn, "container", {"revision": revision, "parent_id": SERATO_LIBRARY_ROOT_CONTAINER_ID, "name": crate_name, "type": 1, "list_order": next_order, "space_id": SERATO_LIBRARY_SPACE_ID, "time_added": now, "expanded": 0, "portable_id": ""})
...
def _asset_values(track: dict[str, Any], revision: int, now: int) -> dict[str, Any]:
    ...
    return {"revision": revision, "portable_id": str(track["serato_portable_id"]), "file_name": path.name, "file_size": path.stat().st_size if path.exists() else 0, "type": "audio", "format": path.suffix.lower().lstrip("."), "artist": track.get("artist", ""), "comments": "", "name": track.get("title", ""), "album": "", "genre": "", "key": "", "bpm": None, "length_ms": None, "time_added": now, "time_modified": now, "analysis_flags": 0, "architectures": 0}
...
def _upsert_space_asset(conn: sqlite3.Connection, asset_id: int) -> int:
    ...
    existing = conn.execute("SELECT id FROM space_asset WHERE asset_id = ? AND space_id = ?", (asset_id, SERATO_LIBRARY_SPACE_ID)).fetchone()
    return int(existing[0]) if existing else dynamic_insert(conn, "space_asset", {"id": asset_id, "asset_id": asset_id, "space_id": SERATO_LIBRARY_SPACE_ID})


def _insert_container_asset(conn: sqlite3.Connection, container_id: int, space_asset_id: int, list_order: int, revision: int, now: int) -> None:
    require_columns(conn, "container_asset", ("container_id", "space_asset_id"), label="Serato")
    dynamic_insert(conn, "container_asset", {"revision": revision, "container_id": container_id, "space_asset_id": space_asset_id, "list_order": list_order, "time_added": now})
