...
def build_bad_paths_plan(snapshot_path: Path, markers: tuple[str, ...] = DEFAULT_BAD_PATH_MARKERS) -> PlanReport:
    ...
    actions = [
        _bad_path_action(row, find_bad_path_marker(row.get("path", ""), markers=markers))
        for row in rows
...
def _bad_path_action(row: dict[str, str], marker: str | None) -> PlanAction | None:
    ...
    evidence.extend(
        label
        for label, present in (("cue_bearing", cue_count > 0), ("playlist_referenced", playlist_count > 0))
        if present
    )
    return PlanAction(
        ...
        if risky
        else "Track points at a bad/staging path but has no cues or playlist references in the snapshot.",
...
def build_audio_compatibility_plan(
    probe_csv_path: Path, profile: AudioCompatibilityProfile | None = None
) -> PlanReport:
    actions = []
        ...
        if result.issues:
            ...
            actions.append(
                PlanAction(
                    ...
                    if has_failure
                    else "Audio probe metadata has warnings that should be reviewed before relying on the file.",
    ...
    for issue in compare_exports(baseline_xml, final_xml).issues:
        ...
        actions.append(
            PlanAction(
                ...
                if issue.code == "cue_not_covered"
                else "The final export appears to have fewer hotcues for this material track.",
