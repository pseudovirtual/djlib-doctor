...
- Do not write to Rekordbox except through `install rekordbox-db` after `stage rekordbox-db-import` or `stage rekordbox-db` prints an exact install token.
- Do not write to Serato except through `install serato-stage` or `install serato-tags` after staging.
- Do not move, convert, delete, or overwrite music files except through `install file-ops`.
...
4. For staged writes, read the generated manifest and only install with the exact token after the user approves.
5. If a Rekordbox DB schema is unsupported, stop and report that the importer failed closed.
