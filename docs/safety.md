...
Rekordbox conversion stages also update a staged `master.db` copy and staged ANLZ `.DAT`/`.EXT` cue and beatgrid files before install. Rekordbox 7.2.8 validation confirmed Rekordbox >=7 ignores AAC gapless metadata in this path, so the default `--cue-shift auto` policy applies a positive net target-minus-source decoder-delay shift to `master.db` cues, ANLZ PCOB/PCO2 cues, and ANLZ PQTZ/PQT2 beatgrid millisecond fields. `--cue-shift none` keeps stored positions unchanged and should be used only after validating that the target Rekordbox/player workflow honors gapless priming metadata. `install rekordbox-convert` verifies the token, source DB hash, staged DB hash, audio hashes, ANLZ hashes, backups, sidecars, and app-closed checks before writing.
