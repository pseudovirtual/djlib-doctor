...
class PlanTests(unittest.TestCase):
    ...
    def test_plan_cli_writes_and_explain_reads_plan(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with contextlib.redirect_stdout(stdout):
                exit_code = main(["plan", "missing-files", "--snapshot", str(snapshot.snapshot_path), "--out", str(plan_path)])
    ...
    def test_duplicates_plan_cli_writes_plan(self):
        with TemporaryDirectory() as tmpdir:
            ...
            stdout = io.StringIO()
            with contextlib.redirect_stdout(stdout):
                exit_code = main(["plan", "duplicates", "--snapshot", str(snapshot.snapshot_path), "--out", str(plan_path)])
    ...
    def test_cues_plan_uses_compare_issues(self):
        report = build_cues_plan(FIXTURE, Path(__file__).parent / "fixtures" / "rekordbox" / "final_missing_material.xml")
    ...
    def test_cues_plan_cli_writes_plan(self):
        with TemporaryDirectory() as tmpdir:
            ...
            stdout = io.StringIO()
            with contextlib.redirect_stdout(stdout):
                exit_code = main(["plan", "cues", "--baseline", str(FIXTURE), "--final", str(final_xml), "--out", str(plan_path)])
