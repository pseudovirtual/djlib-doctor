...
from helpers import fake_mutagen
...
class SeratoAudioTagsTests(unittest.TestCase):
    ...
    def test_write_tags_uses_real_serato_markers2_geob_container_for_all_formats(self):
        ...
        with TemporaryDirectory() as tmpdir, fake_mutagen() as written:
    ...
    def test_id3_formats_share_one_geob_writer_path(self):
        from djlib_doctor import serato_audio_tags

        source = inspect.getsource(serato_audio_tags._write_tags)

        self.assertEqual(source.count("_write_id3_audio("), 2)
        self.assertNotIn("GEOB(", source)
